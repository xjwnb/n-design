...
function Collapse(Props: IProps) {
  const { children, defaultActiveKey = [], accordion = false, ghost = false, onChange } = Props;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className={[Style.n_collapse_pancel_header_title].join(' ')}>{header}</span>
      </div>
      <div
      ...
      >
        <div className={[Style.n_collapse_pancel_content].join(' ')}>{children}</div>
      </div>
    </div>
  );
}
